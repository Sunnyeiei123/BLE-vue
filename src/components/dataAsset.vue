      ...
      <v-dialog v-model="updateDialog" max-width="600px">
        <v-card :class="[isEdit ? 'update-card' : 'add-card']">
          ...
          <v-card-text>
            <v-container>
              <v-row>
                ...
                <v-col cols="12">
                  <v-select v-model="updatedItem.assetType" :items="categories" label="Select Type"></v-select>
                </v-col>
                <v-col cols="12">
          ...
          <v-card-actions :class="[isEdit ? 'update-card-actions' : 'add-card-actions']">
            <v-btn :class="[isEdit ? 'save-btn white-text' : 'add-card-title']" @click="saveItem">{{ isEdit ? 'Save' :
              'Add'
              }}</v-btn>
...
export default {
      ...
      headers: [
        { title: 'Mac', value: 'Mac' },
        {
          ...
          children: [
            ...
            { title: 'Type of Asset', value: 'Type' },
  ...
  methods: {
    ...
    openUpdateForm(item) {
      ...
      this.updatedItem.assetType = item.Type;
      this.updateDialog = true;
    },
    // Open add form
    openAddForm() {
      this.isEdit = false;
      this.updatedItem = { ID: '', Mac: '', name: '', Description: '', assetType: '' };
      this.updateDialog = true;
    },
    // Close form dialog
    closeUpdateForm() {
    ...
    async updateItem() {
      try {
        const response = await api.patch(`/tags/update/${this.updatedItem.ID}`, {
          ...
          assetType: this.updatedItem.assetType,
        });
